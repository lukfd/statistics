...
ggsave("grand_grid_first15.jpg", grid_grand, width = w, height = h)
...
grand_discharge_x_year <- grand %>%
  ...
  ggtitle("grand hydrograph")

# with log scale
grand_discharge_x_year_log <- grand %>%
  ggplot() +
  geom_line(aes(x=days,y=discharge.mm.day, group=year, color = year)) +
  theme_minimal() +
  xlab("julian days") +
  ...
  scale_color_brewer(palette="Dark2") +
  ggtitle("grand hydrograph in log scale")

plot_grid(grand_discharge_x_year, grand_discharge_x_year_log, ncol = 1, align = "v")
```
...
#multiplot(grand_discharge_x_year, grand_temp, grand_snow, grand_topsoil, cols=2)
plot_grid(grand_temp, grand_snow, grand_topsoil, grand_pen, labels = c('A', 'B', 'C', 'D'), ncol = 1, align = "v", label_size = 12)
```
...
grand %>%
  ...
  theme_minimal() +
  ggtitle("Grand Forks 2019 discharge", 
